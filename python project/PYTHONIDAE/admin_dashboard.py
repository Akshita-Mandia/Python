...
def addnew():
    root=Tk()
    # define Labels
    l1 = Label(root, text='Stream_Code')
    l1.grid(row=0, column=0)

    l2 = Label(root, text='Stream_Name')
    l2.grid(row=0, column=2)

    l3 = Label(root, text='Total_Students')
    l3.grid(row=1, column=0)

    l4 = Label(root, text='Practicals(500)')
    l4.grid(row=1, column=2)
    
    l5 = Label(root, text='Viva(200)')
    l5.grid(row=2, column=0)

    l6 = Label(root, text='Theory(500)')
    ...
    def returning():
            ...
            try:
                ...
                if(cnt==1):
                    df=pd.read_csv(filename,encoding="ISO-8859-1",index_col="Stream_Code",error_bad_lines=False)
                    #print(df)
                    viewall()
                    sales_report = pd.pivot_table(df, index=["Stream_Name"], values=["Total_Students","Practicals(500)", "Viva(200)","Theory(500)"],\
                    ...
                    graph1=sns.lmplot('Practicals(500)', 'Viva(200)', data=df,\
                    hue='Stream_Name', fit_reg=False,palette="Set1",height=3,aspect=1.7)
    ...
    def returning():
        ...
        try:
            ...
            if cnt==1 or findings==[]:
                ...
                data.set('Stream_Code\tStream_Name\tTotal_Students\tPracticals(500)\tViva(200)\tTheory(500)\n')
                for i in findings:
...
class ADMIN_DASHBOARD:
    def __init__(self, top=None):
        ...
        top.configure(background="#0BFF73")
